...
- [ ] 新增 `src/pipeline/multi_hop_rag.py`
  - LLM 将复杂问题分解为 2-3 个子问题
  - 每个子问题独立检索，合并去重 context
  - 最终 LLM 综合子问题和 context 生成答案
- [ ] Streamlit 侧边栏新增 Pipeline 模式切换（Single-hop / Multi-hop）
- [ ] 预设问题模式下展示：子问题分解过程 + 各子问题检索来源
...
- [ ] 定义 `RAGState`（TypedDict）：`question / sub_questions / retrieved_docs / answer / hop_count`
- [ ] 实现四个节点：
  - `decompose_node`：问题分解
  - `retrieve_node`：检索（可被循环调用）
  - `judge_node`：判断信息是否充足（初期用规则，后期换 LLM）
  - `generate_node`：生成最终答案
- [ ] `judge_node` → 条件边：`hop_count < 3` 且信息不足 → 回到 `retrieve_node`；否则 → `generate_node`
- [ ] 新增 `src/pipeline/agent_rag.py`，封装 LangGraph 调用接口
- [ ] Streamlit Pipeline 模式新增 Agent 选项，展示推理轨迹（每跳的 query 和来源）
